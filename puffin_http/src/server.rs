...
impl Server {
    ...
    pub fn new(bind_addr: &str) -> anyhow::Result<Self> {
        ...
        let (sender_client_connect, recv_client_connect) = flume::bounded(0); //rendevous channel
        ...
        let join_handle = std::thread::Builder::new()
            ...
            .spawn(move || {
                Server::run(bind_addr, rx, num_clients_cloned, recv_client_connect).unwrap();
            })
            ...
            match self.tcp_listener.accept().await {
                Ok((tcp_stream, client_addr)) => {
                    ...
                    match self.recv_client_connect.recv() {
                        Ok(()) => {}
                        Err(err) => {
                            log::info!("failed to sync rendezvous channel : {}", err);
                        }
                    }
                }
